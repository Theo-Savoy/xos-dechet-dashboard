...
**Dernière synchro statut** : 2026-07-11 soir (vagues de stabilisation Call Manager PRs #25-#27 : moteur/latence/UX, refactor `api/calls.js` → modules `api/_calls/`, consolidation Vercel **C** appliquée → 7 fonctions / 5 slots libres, auto-map `sf_user_id` livré — migration 013).
...
- **Lot 8.1 — OAuth Salesforce (login + liaison user)** — ⬜ : flow Web Server OAuth SF (consumer key/secret déjà dispo), start + callback réels sur `api/auth.js` (`?flow=salesforce` / `?flow=salesforce-callback`), échange code→token, résolution du user SF, upsert `profiles` (email, `sf_user_id`, refresh token chiffré AES-GCM via WebCrypto). Le login SF ouvre une session X OS (JWT Supabase) via l'email SF vérifié — mécanisme : `auth.admin.generateLink(magiclink)` côté serveur + `verifyOtp(token_hash)` côté client. *Aujourd'hui : stub `GET /api/auth?flow=salesforce` → redirect `/?auth_error=sf_coming_soon` (consolidation C).*
  - **Confiance (évaluée 2026-07-11)** : login + mapping ≈ **80-85 %** ; la brique la moins standard est l'émission de session Supabase depuis un OAuth custom (workaround generateLink éprouvé mais sensible aux versions). **Prérequis humain : ajouter l'URL de callback sur la Connected App SF** (Setup, 2 min).
- **Lot 8.1b — Attribution niveau 2 (écritures sous l'identité du commercial)** — ⬜ : les endpoints d'écriture utilisent le refresh token SF **du commercial** quand il existe (fallback utilisateur d'intégration) → `CreatedById` correct dans SF. Cache token par user + retry 401 (même pattern que le cache intégration). Confiance ≈ **75 %** (surface : tous les chemins d'écriture ; risque principal : politiques de session de l'org sur les refresh tokens par user).
...
1. ~~**Consolidation Vercel B**~~ ✅ puis ~~**C** (`launcher`, `auth`)~~ ✅ — 5 slots libres.
2. **2.3 Hub** (spec prête, rôles actés) ∥ **3.1/3.2 Weekly Perf** (spec figée) — aucun prérequis humain.
3. **8.1 OAuth SF** dès que l'URL de callback est posée sur la Connected App (action Théo, 2 min) ; **8.1b attribution niveau 2** dans la foulée.
