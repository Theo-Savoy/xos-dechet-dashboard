...
**Dernière synchro statut** : 2026-07-10 (post Call Manager v2 + polish UI + login dual-option prêt OAuth).
...
### Lot 2.3 — App Hub — ⬜ *(bloqué : liste emails managers)*
...
### Lot 3.0 — Audit métriques activités — 🟡 *(rapport livré ; définitions à valider)*
...
- Livrable : `docs/audits/lot-3.0-metriques-activite.md`.
...
| Hub (2.3) | Bloqué liste managers | Débloquer dès emails managers fournis |
...
| 2 | 2.1 → 2.2, ∥ 2.3 | 🟡 (2.3 ⬜) | 2-3 agents | Launcher OK ; Hub en attente |
| 3 | 3.0 → 3.1 → 3.2 | 🟡 (3.0 rapport ; 3.1/3.2 ⬜) | séquentiel (audit d'abord) | Weekly Perf |
...
1. **8.1** — brancher OAuth SF (UI déjà prête) → débloque attribution / mapping `sf_user_id` propre.
2. **3.0 validation Théo** → **3.1 / 3.2** Weekly Perf (valeur manager).
3. **2.3 Hub** dès liste managers.
4. **5 / 6 / 7** selon priorité produit (Arena vs Business Review vs Agent).
...
- Validation des définitions de métriques (lot **3.0** — 4.0 déjà livré côté Call Manager).
