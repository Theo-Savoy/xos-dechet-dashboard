...
**Dernière synchro statut** : 2026-07-11 nuit (Hub 2.3 livré PRs #29-#32 ; `api/perf` 3.1 livré ; OAuth SF 8.1/8.1b/8.2 en prod — provider refresh token auto-bridgé via `POST /api/auth`, notification de liaison, statut SF en barre de menu ; template email magic-link ; lot 3.2 UI Weekly dispatché ; **Phase 9 Copilot ajoutée au plan** — périmètre validé Théo, contrat `docs/specs/copilot.md`).

## Organisation

- **Coordination** : Claude (Fable) — spécification des lots, dispatch, suivi, contrôle qualité, merges. **N'écrit pas le code des lots.**
- **Exécution** : agents via Orca — Cline, Command Code, Antigravity, Claude Code — tous sur **Sonnet 5 Medium**. L'assignation indiquée par lot est indicative (équilibrage de charge) ; tout agent peut prendre tout lot, la spec est la frontière. *Depuis le 2026-07-11, les vagues de stabilisation/optimisation passent par **Codex CLI (GPT-5.6 Terra, effort medium)** briefé et contrôlé par Claude en direct — même gate QC.*
...
- `api/search.js` : SOSL multi-objets (Account, Contact, Opportunity), JWT requis, pas de cache. *(Consolidé dans `api/launcher.js` le 2026-07-11 — consolidation C.)*
...
- `api/log.js` : création de Task SF rattachée (compte/contact/opp) avec mention "via X OS par {nom}" + entrée `action_journal` ; création express de **Contact** (`/create`) ; `/clean` ouvre le Cleaner pré-filtré (query param lu par l'iframe — seule évolution tolérée côté `dashboard.html`, à défaut on ouvre sans pré-filtre). *(Consolidé dans `api/launcher.js` le 2026-07-11 ; Tasks/Contacts créés avec `OwnerId` = commercial connecté.)*
...
### Lot 2.3 — App Hub — ✅ *(livré 2026-07-11, PRs #29/#31/#32 — Hub réservé manager/admin dans le dock, déconnexion en barre de menu, label quota SF lisible)*
...
## Phase 3 — Weekly Perf — 🟡 *(API livrée ; UI en cours — lot 3.2 dispatché le 2026-07-11 soir)*
...
### Lot 3.2 — UI Weekly Perf — ✅ *(livré 2026-07-11 — Codex Terra medium, mergé sur main : Pulse cards + sparklines, chart Généré vs Gagné (Recharts), Effort, vues Moi/Équipe, 404 tests)*
...
### Lot 3.3 — Rituel équipe (remplace le Google Sheet) — ⬜ *(spec : `weekly-perf.md` § 9 — ajouté 2026-07-11)*
...
- **Auto-map `sf_user_id`** — ✅ *(2026-07-11, migration 013)* : table `sf_user_map` (email → User SF, seedée depuis l'org), lookup dans `handle_new_user` à la création du profil, backfill des profils existants. Les 4 commerciaux seront mappés automatiquement à leur premier login.

### Vagues de stabilisation Call Manager *(2026-07-11, PRs #25-#27 — audit Fable 5, exécution Codex GPT-5.6 Terra)* — ✅
...
## Phase 6 — Business Review — ⬜ *(re-spécifiée 2026-07-11 : cockpit macro + partage d'analyses — contrat `docs/specs/business-review.md`)*
...
- **Lot 6.0 — Audit macro** — ⬜ : FY XOS, définitions CA signé, profondeur N-2, valeurs picklist réelles, volumétrie SOQL ; réutilise les `references/` du skill Hermes (SOQL + pièges déjà documentés : OwnerId vs CreatedById, ISO week, double comptage Global, Sur-mesure 6 mois glissants). **Validation Théo** — bloque 6.1. *(L'ancien volet OpportunityLineItem/Product2 + motifs gain/perte reste au périmètre de l'audit.)*
...
- **Lot 8.1b — Attribution niveau 2 (écritures sous l'identité du commercial)** — 🟡 *(code prêt, activation prod à valider)* : Call Manager, Launcher et Cleaner utilisent le refresh token SF **du commercial** quand il existe, avec fallback intégration. Cache 30 min par user + retry 401 sous la même identité ; le Cleaner exige désormais le JWT X OS et le reçoit par `postMessage` same-origin. **Reste** : lier un compte test et vérifier `CreatedById` en live.
...
## Phase 9 — Copilot *(pilotage pipeline & adoption CRM)* — ⬜
...
### Lot 9.0 — Audit SOQL Copilot — ⬜ *(bloque 9.1)*
...
| 3 | 3.0 → 3.1 → 3.2 | 🟡 (3.0/3.1 ✅ ; 3.2 en cours) | séquentiel | Weekly Perf — API en prod |
...
2. **3.2 UI Weekly Perf** — en cours (agent dispatché, spec `docs/specs/weekly-perf.md` §5).
3. **Smoke-test attribution niveau 2** : login SF réel + vérifier `CreatedById` sur une Task de test (action Théo).
4. **9 Copilot** (9.0 audit → 9.1 API → 9.2 UI) — priorité actée avant Arena (2026-07-11), contrat `docs/specs/copilot.md`.
